...
**Goal:** Rewrite `db/seeds.rb` to programmatically generate 5 years (2021-04 to 2026-03) of realistic, fictional financial data for local dev/demo use.

**Architecture:** Single `db/seeds.rb` file. Helper methods defined at the top handle double-entry transaction creation. A 60-month loop generates transactions with income drift (2%/yr), per-transaction spending noise (±15%), and a 1.3× December multiplier for recreation/restaurant spending. Account balances are recalculated from transaction lines at the end.

**Tech Stack:** Ruby, Rails 8 ActiveRecord, PostgreSQL
...
### Task 1: Teardown + Ledger + User + Accounts

**Files:**
- Rewrite: `db/seeds.rb`

- [ ] **Step 1: Replace the entire contents of `db/seeds.rb`**
...
(2 system + 8 user accounts)

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: teardown, ledger, user, accounts"
```

---

### Task 2: Category Groups, Categories, and Expense Accounts

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Add category constant and creation loop**

Replace the final `puts "Seeded #{Account.count} accounts."` line with:
...
Expected output (last line):
```
Seeded 32 accounts, 22 categories.
```

(10 original + 22 expense accounts = 32; 22 categories across 7 groups)

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: category groups, categories, expense accounts"
```

---

### Task 3: Payee Pool Constants + Payee Creation

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Add payee constants and creation after category creation**

Replace `puts "Seeded #{Account.count} accounts, #{Category.count} categories."` with:
...
Expected output (last line):
```
Seeded 32 accounts, 22 categories, 48 payees.
```

(45 category payees + 3 income payees = 48)

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: payee pool constants and payee creation"
```

---

### Task 4: Transaction Helper Methods

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Add helper methods at the very top of `db/seeds.rb`, before the first `puts`**
...
- [ ] **Step 2: Run seeds and verify (no behavioral change)**

```bash
bin/rails db:seed
```

Expected: same output as Task 3.

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: transaction helper methods"
```

---

### Task 5: Monthly Loop — Income, Mortgage, RRSP, Emergency Fund

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Add the generation loop after payee creation**

Replace `puts "Seeded #{Account.count} accounts, #{Category.count} categories, #{Payee.count} payees."` with:
...
Expected output includes progress lines and ends with approximately:
```
Generated 245 transactions so far.
```

(120 income + 60 mortgage + 60 emergency fund + 5 RRSP = 245)

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: monthly loop with income, mortgage, RRSP, emergency fund"
```

---

### Task 6: Monthly Fixed Bills

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Add fixed monthly bills inside the loop, after the RRSP block and before the CC spend tracking**

Insert between the RRSP block and `prev_northbrook_cc_spend = curr_northbrook_cc_spend`:
...
Expected output (last line):
```
Generated 1023 transactions so far.
```

(245 from Task 5 + 118 CC payments + 660 fixed bills = 1023)

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: monthly fixed bills"
```

---

### Task 7: Monthly Variable Expenses

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Add variable expenses inside the loop, after the utilities block and before the CC spend tracking**
...
```ruby
  ...
  rand(0..3).times do
    ...
    day += 1 while Date.new(year, month, day).saturday? || Date.new(year, month, day).sunday?
...
Expected output (last line):
```
Generated ~3200 transactions so far.
```

(Exact count varies due to randomness; expect 3000–3500)

- [ ] **Step 3: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: monthly variable expenses"
```

---

### Task 8: Account Balance Sync + Final Output + Verify

**Files:**
- Modify: `db/seeds.rb`

- [ ] **Step 1: Replace `puts "Generated #{TransactionEntry.count} transactions so far."` with balance recalculation and final summary**
...
- [ ] **Step 2: Run the full seed and verify**
...
Done. Seeded:
  1 ledger, 1 user
  32 accounts, 22 categories, 48 payees
  ~3200 transactions (~6400 lines)

Log in at http://localhost:3000 with demo@example.com / password
```

- [ ] **Step 3: Verify the app works with seed data**
...
Open `http://localhost:3000`, log in as `demo@example.com` / `password`. Verify:
- Account list shows 6 user accounts with non-zero balances
- Register view for Northbrook Chequing shows ~5 years of transactions
- Transactions have a mix of statuses (uncleared, cleared, reconciled)

- [ ] **Step 4: Commit**

```bash
git add db/seeds.rb
git commit -m "seeds: account balance sync and final output"
```
...
- **Retirement category:** Created in the taxonomy but not directly used by any transaction — the RRSP account handles retirement savings as a transfer. This mirrors how the app would work in practice.
- **Mortgage/Rent, Emergency Fund categories:** Same as above — these categories exist for budgeting purposes but the actual transactions are modelled as transfers between accounts.
- **Re-seeding:** Running `bin/rails db:seed` again is safe — the teardown block at the top clears everything first.
- **Performance:** Generating ~3200 transactions with individual AR creates takes ~15–30 seconds. Acceptable for a dev seed.
